...
RetroPie-X86 transforms a bare Ubuntu 24.04 LTS server install into a dedicated retro gaming console — no desktop environment, no display manager, no bloat. It boots silently and directly into EmulationStation, drives the latest Intel, NVIDIA, and Vulkan GPU stacks, shares your ROM library over NFS, and gets out of your way.
...
> The log is written to `/var/log/retropie-x86-install.log`.
...
RetroPie-X86 supports a flexible NFS setup for sharing ROMs across multiple machines.
...
```bash
sudo ~/retropie-x86/scripts/03-video-drivers.sh
```

**Change display resolution:**
```bash
# Edit retropie.cfg, then re-run:
sudo ~/retropie-x86/scripts/02-dependencies.sh
sudo ~/retropie-x86/scripts/05-grub-splash.sh
...
| Area | MizterB original | RetroPie-X86 |
|---|---|---|
