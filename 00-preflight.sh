...
if command -v lspci &>/dev/null; then
    ...
    if [[ "${GPU_DRIVER:-auto}" == "auto" ]]; then
        if echo "$GPU_INFO" | grep -qi "nvidia"; then
            ...
            echo "GPU_DRIVER_RESOLVED=nvidia" >> /tmp/retropie-gpu.env
        elif echo "$GPU_INFO" | grep -qi "intel"; then
            info "Auto-detected: Intel GPU → will install Mesa (iris/xe)"
            echo "GPU_DRIVER_RESOLVED=intel" >> /tmp/retropie-gpu.env
        elif echo "$GPU_INFO" | grep -qi "amd\|radeon\|advanced micro"; then
            info "Auto-detected: AMD GPU → will install Mesa (radeonsi)"
            echo "GPU_DRIVER_RESOLVED=amd" >> /tmp/retropie-gpu.env
        else
            warn "Could not auto-detect GPU type. Video drivers will be skipped."
            echo "GPU_DRIVER_RESOLVED=none" >> /tmp/retropie-gpu.env
        fi
    else
        echo "GPU_DRIVER_RESOLVED=${GPU_DRIVER}" >> /tmp/retropie-gpu.env
    fi
else
...
echo "PREFLIGHT_OK=true" > /tmp/retropie-preflight.env
